...
class SAQIMod(loader.Module):
    ...
    async def ihborcmd(self, message: "telethon.tl.types.Message") -> None:
        """Special animation"""
        if not message.out:
            message = await message.respond("ihbor")
        ...
        for i in range(8):
            ...
            await sleep(0.3)
        for i in ["Вс", "Всем", "Всем при", "Всем привет🌝"]:
